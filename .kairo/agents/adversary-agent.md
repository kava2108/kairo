...
6. **FAIL 次元の自動ルーティング決定**
   ...
   | FAIL 次元 | ルーティング先 |
   |---|---|
   | D1 Spec Fidelity | `blocked:imp` + `/kairo:implement --update` |
   | D2 Edge Case Coverage | `blocked:imp` + `/kairo:test` |
   | D3 Implementation Correctness | `blocked:imp` + `/kairo:implement --update` |
   | D4 Structural Integrity | `blocked:imp` + `/kairo:imp_generate --update` |
   | D5 Verification Readiness | `blocked:imp` + `/kairo:test` |
