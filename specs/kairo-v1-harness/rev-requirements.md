---
kairo:
  node_id: "rev-requirements:kairo-v3-harness"
...
coherence:
  id: "rev-requirements:kairo-v3-harness"
  depends_on:
    - id: "imp:kairo-v3-harness"
      relation: "reverse_of"
      confidence: 0.90
      required: false
    - id: "test-plan:kairo-v3-harness"
...
- 実装コード: `.kairo/hooks/post-tool-use.sh` L44-52、`.github/workflows/vckd-pipeline.yml`
- 信頼性: 🔵
...
- 実装コード: `.kairo/lib/phase-gate.sh` `dispatch_baton()` L333-356
- 信頼性: 🔵
...
- 実装コード: `.kairo/lib/phase-gate.sh` `emit_baton()` L410-449
- 信頼性: 🔵
...
- 実装コード: `.kairo/lib/phase-gate.sh` `_get_config()` L20-30（デフォルト値引数）
- 信頼性: 🔵
...
- 実装コード: `.kairo/lib/phase-gate.sh` `check_phase_gate()` L201、`_check_artifacts()`
- 信頼性: 🔵
...
- 実装コード: `.kairo/lib/phase-gate.sh` `_check_ceg()` / `commands/coherence-scan.md` Step4
- 信頼性: 🔵
...
- 実装コード: `.kairo/lib/phase-gate.sh` `_append_transition()` L103-110（mktemp + mv パターン）
- 信頼性: 🔵
...
- 実装コード: `.kairo/agents/adversary-agent.md`（強制ネガバイアスルール）
...
- 実装コード: `.kairo/hooks/post-tool-use.sh` L22（`[[ =~ ^[0-9]+$ ]]`）
- 信頼性: 🔵
...
- 実装コード: `.kairo/lib/phase-gate.sh` `_gh_with_retry()` L63-80
- 信頼性: 🔵
...
- 実装コード: `.kairo/lib/phase-gate.sh` `_ensure_baton_log()` L51-59
- 信頼性: 🔵
...
**EARS 記法**:  
[WHEN] `.vckd/config.yaml` の `harness.enabled` が `false` のとき [THE SYSTEM SHALL] 全バトン関連関数が early-return し、GitHub への一切の操作を行わない（kairo v2.x 互換）

**根拠**:
- テストコード: `T03/testcases.md` TC-T03-08
- 実装コード: `.kairo/lib/phase-gate.sh` `_check_harness_enabled()` L32-37（全 public 関数の先頭で呼ぶ）
- 信頼性: 🔵
...
| IMP 要件 | 状態 | 推奨アクション |
|---|---|---|
| `phases.json` によるフェーズ固有チェック定義（IMP T03 §1.4） | 🟡 実装の中で JSON ファイル参照は確認できず Bash 関数内に直接実装か | `/kairo:drift_check kairo-v3-harness` で確認 |
