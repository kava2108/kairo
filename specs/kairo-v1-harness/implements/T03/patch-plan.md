---
kairo:
  node_id: "impl:kairo-v3-harness:T03"
...
coherence:
  id: "impl:kairo-v3-harness:T03"
  depends_on:
    - id: "imp:kairo-v3-harness"
      relation: "implements"
      confidence: 0.95
      required: true
    - id: "impl:kairo-v3-harness:T01"
      relation: "depends_on"
      confidence: 1.0
      required: true
  band: "Green"
...
**Issue**: [#3](https://github.com/kava2108/kairo/issues/3)  
**IMP バージョン**: 1.1.0  
**実装日**: 2026-04-05
...
| ファイル | 操作 | 状態 |
|---------|------|------|
| `.kairo/lib/phase-gate.sh` | 新規作成 | ✅ 実装済み |
| `.kairo/lib/on-label-added.sh` | 新規作成 | ✅ 実装済み |

---

## 実装内容

### `.kairo/lib/phase-gate.sh` — 公開 API
...
**起因**: `/kairo:review kairo-v3-harness --adversary` による D1 / D4 FAIL 検出

#### 変更対象ファイル（--update）

| ファイル | 操作 | 修正起因 |
|---------|------|---------|
| `.kairo/lib/phase-gate.sh` | 更新（`_check_ceg()` 置き換え） | Adversary D1: REQ-003-AC-2 未充足 |
| `.kairo/lib/on-label-added.sh` | 更新（`on_label_added()` SRP 分割） | Adversary D4: 単一責任原則違反 |
