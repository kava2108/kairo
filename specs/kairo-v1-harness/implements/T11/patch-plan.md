---
kairo:
  node_id: "impl:kairo-v3-harness:T11"
...
# Patch Plan: kairo-v3-harness / T11
...
| ファイル | 操作 | 説明 |
|---|---|---|
| `.kairo/lib/phase-gate.sh` | **変更** | `_validate_from_phase()` 関数を追加、`check_phase_gate()` 冒頭で呼び出し |
| `.kairo/hooks/post-tool-use.sh` | **変更** | `VCKD_FROM_PHASE` 許可リスト検証を追加（空文字チェック直後） |
| `specs/kairo-v1-harness/tests/T11/test_vckd_from_phase.sh` | **新規作成** | 7件の自動テスト（PASS=7 / FAIL=0） |
| `specs/kairo-v1-harness/implements/T11/patch-plan.md` | **新規作成** | このファイル |

---

## 実装詳細

### `.kairo/lib/phase-gate.sh` の変更
...
### `.kairo/hooks/post-tool-use.sh` の変更
...
**ファイル**: `specs/kairo-v1-harness/tests/T11/test_vckd_from_phase.sh`  
