...
**対象 Issue**: [#11](https://github.com/kava2108/kairo/issues/11)  
...
フェーズ遷移ルールを `.kairo/config/phases.json` から `jq` で読み込む構造に移行するタスクである。
...
- **作成**: `.kairo/config/phases.json` — 5 遷移（REQ→TDS / TDS→IMP / IMP→TEST / TEST→OPS / OPS→CHANGE）を定義
...
| ファイル | 操作 | 変更内容 |
|---------|------|---------|
| `.kairo/lib/phase-gate.sh` | 更新 | `_check_phase_specific()` を JSON 対応に改訂、`_run_phase_check()` を追加 |
| `.kairo/config/phases.json` | 新規作成 | フェーズ遷移定義 JSON（5 遷移） |
...
| `.kairo/config/phases.json` を `jq` で参照 | `_check_phase_specific()` 内で `jq` 使用 | ✅ |
...
| `BASH_SOURCE[0]` が空（対話シェル等） | CWD 基準の `.kairo/config/phases.json` にフォールバック |
...
| ファイル配置 | `.kairo/config/phases.json` は新規ディレクトリ（`.kairo/config/`）に配置済み。既存のディレクトリ構造と整合的 |
...
1. **T13 固有テストの追加（任意）**: `phases.json` 不在時・不正 JSON 時の FAIL を直接確認するテストケースを `specs/kairo-v1-harness/tests/T13/testcases.sh` として追加することを推奨する（優先度: Low）。
