...
---
kairo:
  node_id: "sync-report:kairo-v3-harness"
  artifact_type: "sync-report"
  phase: "OPS"
...
coherence:
  id: "sync-report:kairo-v3-harness"
  depends_on:
    - id: "imp:kairo-v3-harness"
      relation: "validates"
      confidence: 1.0
      required: true
    - id: "drift-report:kairo-v3-harness"
      relation: "references"
      confidence: 1.0
      required: false
...
| 項目 | 結果 | 詳細 |
|---|---|---|
| issue-struct.md の存在 | ⚠️ | `specs/kairo-v1-harness/issue-struct.md` 不在。本プロジェクトは kiro 統合（`.kiro/specs/`）のため Issue 成果物は `design.md` / `requirements.md` に相当 |
| tasks.md の存在 | ⚠️ | `specs/kairo-v1-harness/tasks.md` 不在。タスク定義は IMP.md §7 に直接記載（T01〜T09） |
| IMP.md の存在 | ✅ | `.kiro/specs/kairo-v1-harness/IMP.md`（非標準パス）に 1.1.0 存在 |
| issue_id の一致 | ✅ | "kairo-v3-harness" — IMP / 全 patch-plan / 全 testcases / テスト計画書で一致 |
...
→ `specs/kairo-v1-harness/sync-actions.md` を参照
