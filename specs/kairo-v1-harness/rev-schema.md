---
kairo:
  node_id: "rev-schema:kairo-v3-harness"
  artifact_type: "rev_schema"
  phase: "OPS"
  issue_id: "kairo-v3-harness"
  created_at: "2026-04-05T00:00:00Z"
coherence:
  id: "rev-schema:kairo-v3-harness"
  depends_on:
...
# 逆生成スキーマ仕様書: kairo-v3-harness

> kairo v3.0 のデータスキーマは YAML / JSON / Markdown frontmatter の 3 形式で構成されます。
...
.vckd/config.yaml          設定スキーマ（静的）
       │
       ├── harness.enabled ─ .kairo/lib/phase-gate.sh
       └── harness.AUTO_STEP
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L20-48, `.vckd/config.yaml`
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L50-110（`_ensure_baton_log`, `_append_transition`）
...
```json
{
  ...
  "nodes": {
    "<coherence.id>": {
      "artifact_type": "imp",        // string: IMP の kairo.artifact_type
      "phase": "IMP",               // string
  ...
  "edges": [
    {
      "from": "imp:kairo-v3-harness",
      "to": "req:kairo-v3-harness",
      "relation": "implements",
      "confidence": 0.95,
      "required": true
    }
...
---
kairo:
  node_id: string           # "<type>:<issue-id>[:<task-id>]" 例: "imp:kairo-v3-harness"
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `specs/kairo-v1-harness/implements/T01/patch-plan.md`（実際の frontmatter 例）
