...
# Issue 構造定義: kairo-v3-harness

## 0. Summary

kairo AI-TDD エンジンに VCKD v3 Phase Gate / Baton Harness を実装し、
GitHub Issues のフェーズラベル遷移を自動化・可視化する。

## 1. 背景・動機（Why）

kairo は Issue → IMP → 実装 → テスト → 逆仕様 → 同期 の全フェーズを一貫して実行する AI-TDD エンジンである。
...
- kairo コア命令（`imp_generate`, `implement` 等）の変更
