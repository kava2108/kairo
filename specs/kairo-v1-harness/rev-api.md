---
kairo:
  node_id: "rev-api:kairo-v3-harness"
  artifact_type: "rev_api"
  phase: "OPS"
  issue_id: "kairo-v3-harness"
  created_at: "2026-04-05T00:00:00Z"
coherence:
  id: "rev-api:kairo-v3-harness"
  depends_on:
    - id: "imp:kairo-v3-harness"
      relation: "reverse_of"
      confidence: 0.90
      required: false
    - id: "rev-spec:kairo-v3-harness"
      relation: "depends_on"
      confidence: 0.90
      required: false
  band: "Green"
...
# 逆生成 API 仕様書: kairo-v3-harness

> kairo v3.0 の「API」は HTTP ではなく、以下の 3 層から構成されます：
...
| コマンド | 引数 | 説明 | 実装ファイル |
|---------|------|------|------------|
| `/kairo:install` | `[--harness] [--project]` | 初期セットアップ。`--harness` でバトンインフラを構築 | `commands/install.md` |
| `/kairo:baton-status` | `[issue-id]` | バトン遷移状態を 4 カテゴリで表示 | `commands/baton-status.md` |
| `/kairo:coherence-scan` | `[issue-id]` | frontmatter を収集して coherence.json を再構築 | `commands/coherence-scan.md` |
| `/kairo:rescue` | `<issue-id> [--reason <text>]` | ブロック解除・リトライカウントリセット | `commands/rescue.md` |
| `/kairo:imp_generate` | `<issue-id> [--update]` | IMP の生成・更新（CEG frontmatter 付与） | `commands/imp_generate.md` |
| `/kairo:implement` | `<issue-id> [task-id]` | patch-plan.md 生成（CEG frontmatter 付与） | `commands/implement.md` |
| `/kairo:test` | `<issue-id> [--vmodel all]` | testcases.md・test-plan.md を生成 | `commands/test.md` |
| `/kairo:rev` | `<issue-id> [--target all]` | 実装から逆仕様書を生成 | `commands/rev.md` |
| `/kairo:drift_check` | `<issue-id>` | 仕様と実装の乖離を検出・可視化 | `commands/drift_check.md` |

---

## 2. Bash 公開関数（`.kairo/lib/phase-gate.sh`）
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L201
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L333
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L357
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L410
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L450
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L163
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L113-161
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/hooks/post-tool-use.sh` L4-8
...
| ラベル | 実行内容 | エージェントファイル |
|--------|---------|-----------------|
| `phase:req` | Claude Code（headless） | `.kairo/agents/requirements-agent.md` |
| `phase:tds` | Claude Code（headless） | `.kairo/agents/design-agent.md` |
| `phase:imp` | Claude Code（headless） | `.kairo/agents/implement-agent.md` |
| `phase:test` | Claude Code（headless） | `.kairo/agents/test-agent.md` |
| `phase:ops` | Claude Code（headless） | `.kairo/agents/ops-agent.md` |
| `phase:change` | Claude Code（headless） | `.kairo/agents/change-agent.md` |
| `approve` | Bash のみ | `.kairo/lib/on-label-added.sh` |
