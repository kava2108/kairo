---
kairo:
  node_id: "rev-spec:kairo-v3-harness"
...
### F-01: Baton Infrastructure セットアップ（`/kairo:install --harness`）
...
6. `.kairo/hooks/post-tool-use.sh` を生成
...
### F-02: PostToolUse Hook（`.kairo/hooks/post-tool-use.sh`）
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/hooks/post-tool-use.sh` L1-60
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L333-356
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L357-409
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L410-449
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/on-label-added.sh`
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L201-332
...
**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L113-200
...
→ kairo v2.x 以前の環境では一切のバトン操作を行わない
```

**IMP との差分**: ✅ 一致  
**実装根拠**: `.kairo/lib/phase-gate.sh` L32-37（`_check_harness_enabled`）
...
全仕様が実装と一致しています。
→ 詳細確認: `/kairo:drift_check kairo-v3-harness`
