...
# 実行: bash specs/kairo-v1-harness/tests/T03/test_phase_gate.sh
#
...
test_tc13_check_phase_gate_fail_circular_dep() {
  ...
  (
    ...
    # OPS フェーズ成果物を用意
    printf "# drift-report\ndrift score: 5\n" > specs/kairo-v1-harness/drift-report.md
...
EOJSON
    ...
    set +e
    check_phase_gate "OPS" "my-feature" "kairo-v3-harness"
    local retval=$?
    echo "retval=$retval gate=${VCKD_GATE_RESULT:-}" >> "$result_file"
  )
