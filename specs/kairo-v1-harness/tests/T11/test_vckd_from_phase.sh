...
# 実行: bash specs/kairo-v1-harness/tests/T11/test_vckd_from_phase.sh
#
# 前提: bash 4.x 以降
# テスト対象:
#   - .kairo/lib/phase-gate.sh  : _validate_from_phase() + check_phase_gate() での呼び出し
#   - .kairo/hooks/post-tool-use.sh : VCKD_FROM_PHASE 許可リスト検証（exit 2）
#
# 設計ポイント:
...
PHASE_GATE_LIB="$REPO_ROOT/.kairo/lib/phase-gate.sh"
POST_TOOL_HOOK="$REPO_ROOT/.kairo/hooks/post-tool-use.sh"
...
_make_test_env() {
  local d
  d=$(mktemp -d)
  mkdir -p "$d/.vckd" "$d/graph" "$d/specs/kairo-v1-harness" "$d/.kiro/specs/my-feature"
...
test_tc05_check_phase_gate_invalid_phase_returns_2() {
  ...
  (
    ...
    set +e
    check_phase_gate "INVALID_PHASE" "my-feature" "kairo-v3-harness"
    echo "retval=$?" >> "$result_file"
  ) 2>/dev/null
