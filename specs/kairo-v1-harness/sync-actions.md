---
kairo:
  node_id: "sync-actions:kairo-v3-harness"
...
# 手動対応アクション: kairo-v3-harness

整合性スコア: **90/100 (Excellent)** — 全アクションは Low 優先度です。  
対応完了後に `/kairo:sync kairo-v3-harness --report-only` で再確認してください。
...
| **背景** | 本プロジェクトは kiro 統合（`.kiro/specs/`）を使用しているため、標準の `specs/kairo-v1-harness/issue-struct.md`・`tasks.md` が生成されていない |
| **対応方法 A** | `specs/kairo-v1-harness/issue-struct.md` を手動で作成し、IMP.md の 23 AC を EARS 記法で転記する（`/kairo:issue_init kairo-v3-harness` を参考に手動作成） |
| **対応方法 B** | IMP.md 冒頭に `.kiro/specs/kairo-v1-harness/design.md` へのリンクを追記し、Issue ソースを明示する |
| **担当** | |
| **期限** | 次回 sync 実行まで |
...
| **背景** | `.kiro/specs/kairo-v1-harness/IMP-checklist.md` の全 20+ 項目が `[ ]` 未チェック |
...
| **対応方法 B** | `.kairo/config/phases.json` を新規作成し、`_check_phase_specific()` から `jq` で読み込む形式に移行する（拡張性が上がる） |
| **コマンド例** | `対応方法 B を選択する場合: /kairo:implement kairo-v3-harness --task T03-patch-phases-json` |
| **担当** | |
| **期限** | 次回 Wave まで |
...
| **対応方法** | テスト環境で `/kairo:pr kairo-v3-harness` を実行し、生成される PR コメントに以下の 4 点が含まれることを確認する： |
