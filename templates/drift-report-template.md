...
### CRITICAL（即時対応）
- AC-003 の実装とテストが存在しない → `/kairo:implement {{issue_id}} TASK-XXXX` を実行する

### WARNING（早期対応）
- AC-002 のテストを追加する → `/kairo:test {{issue_id}}`
- IMP の API 仕様を実装に合わせて更新する → `/kairo:imp_generate {{issue_id}} --update`

### INFO（任意対応）
- TASK-0002 の実装着手 → `/kairo:implement {{issue_id}} TASK-0002`
