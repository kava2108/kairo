...
---
kairo:
  node_id: "imp:kairo-v3-harness"
  artifact_type: "imp"
  phase: "IMP"
  feature: "kairo-v3-harness"
  imp_version: "1.2.0"
  status: "active"
...
coherence:
  id: "imp:kairo-v3-harness"
  depends_on:
    - id: "req:kairo-v3-harness"
      relation: "implements"
      confidence: 0.95
      required: true
    - id: "design:kairo-v3-harness"
...
# Implementation Plan（IMP）: kairo v3.0 Harness Engineering 統合

**Feature**: `kairo-v3-harness`  
**IMP バージョン**: 1.2.0  
**対象 TDS**: `.kiro/specs/kairo-v1-harness/design.md`（Phase Gate TDS→IMP: PASS 済み）  
**作成日**: 2026-04-04  
**AUTO_STEP**: false（Manual Baton Mode 前提）

> **Issue 構造・ラベル定義・フェーズ FSM の詳細**: `.kiro/specs/kairo-v1-harness/design.md` §1〜§3 を参照。  
...
**kairo v3.0 — Harness Engineering 統合**
...
| **P2** | T16 | `req:kairo-v3-harness` ノード登録（Amber 解消） | ✅ 並列可 |
...
「`/kairo:install --harness` を実行すれば即座にバトンが動く」状態を作る。
...
| `.kairo/hooks/post-tool-use.sh` | 新規作成 | VCKD_GATE_RESULT 環境変数を読み `dispatch_baton` を呼ぶシェルスクリプト |
...
**`.kairo/hooks/post-tool-use.sh` の構造**:
...
| 入力 | 型・形式 | 説明 |
|------|---------|------|
| TDS §4.2 frontmatter スキーマ | YAML Schema | kairo: / coherence: / baton: の全フィールド定義 |
...
| `.kairo/templates/imp-template.md` | 新規作成 | frontmatter 付き IMP.md のひな形 |
...
| 出力ファイル | 操作 | 内容 |
|------------|------|------|
| `.kairo/lib/phase-gate.sh` | 新規作成 | `check_phase_gate`, `dispatch_baton`, `emit_pending`, `emit_baton`, `emit_blocked` の実装 |
| `.kairo/lib/on-label-added.sh` | 新規作成 | `approve` ラベル検知 → バトン発行ハンドラ |
...
```bash
# .kairo/lib/phase-gate.sh — _check_phase_specific() の実装方式
case "$from_phase" in
...
| ファイル | 担当エージェント |
|---------|----------------|
| `.kairo/agents/requirements-agent.md` | RequirementsInterviewer + EARSFormatter + Validator |
| `.kairo/agents/design-agent.md` | ArchDesigner + APIDesigner + SchemaDesigner + TaskSplitter |
| `.kairo/agents/implement-agent.md` | IMPGenerator + Implementer |
...
| `.kairo/agents/*.md`（T04, T06 の成果物） | Phase Agent のシステムプロンプト |
...
| ファイル | 担当エージェント |
|---------|----------------|
| `.kairo/agents/test-agent.md` | UnitTestWriter + IntegrationTestWriter + E2ETestWriter + SecurityTestWriter |
| `.kairo/agents/adversary-agent.md` | Adversary（コンテキスト分離・5次元評価） |
| `.kairo/agents/ops-agent.md` | RevExtractor + DriftChecker |
| `.kairo/agents/change-agent.md` | SyncAuditor + PRWriter |
...
```markdown
# .kairo/agents/adversary-agent.md における必須記述
...
`harness.enabled=false` のとき、全コマンドが v1.0（kairo v2 以前）と同一の動作をすることを保証する。
...
| 入力 | 説明 |
|------|------|
| `.kairo/lib/phase-gate.sh`（T03 の成果物） | `enabled` チェックを追加する対象 |
...
| ファイル | 変更内容 |
|---------|---------|
| `.kairo/lib/phase-gate.sh` | 全関数の冒頭に `[[ "$enabled" != "true" ]] && return 0` を追加 |
...
| `.kairo/agents/*.md`（T04, T06 の成果物） | リトライロジックを追記する対象 |
...
| `.kairo/agents/*.md` | 全ファイル更新 | リトライカウント管理・3 回失敗後の escalate 処理を追記 |
...
retries >= 3 の場合:
  ...
  以下の形式でコメントを投稿する:
    ...
    リトライ回数: 3/3
    最後のエラー: <エラー内容>
    推奨アクション: /kairo:rescue <issue-number> を実行してください
  処理を終了する（exit 3）
```
...
| 入力 | 説明 |
|------|------|
| `specs/kairo-v1-harness/tests/T01/testcases.md` | TC-T01-01〜TC-T01-05 |
| `specs/kairo-v1-harness/tests/T04/testcases.md` | TC-T04-01〜TC-T04-04 |
| `specs/kairo-v1-harness/tests/T07/testcases.md` | TC-T07-01〜TC-T07-04 |
| `specs/kairo-v1-harness/tests/T03/test_phase_gate.sh` | 参照パターン（VCKD_TEST_MODE / mock_gh） |

#### 1.3 Outputs

| ファイル | 内容 |
|---------|------|
| `specs/kairo-v1-harness/tests/T01/test_baton_infra.sh` | TC-T01-01〜05 の自動テスト |
| `specs/kairo-v1-harness/tests/T04/test_phase_agent.sh` | TC-T04-01〜04 の自動テスト |
| `specs/kairo-v1-harness/tests/T07/test_coherence_scan.sh` | TC-T07-01〜04 の自動テスト |
...
| ファイル | 変更内容 |
|---------|---------|
| `.kairo/hooks/post-tool-use.sh` | 冒頭に `VCKD_FROM_PHASE` 許可リスト検証を追加 |
| `.kairo/lib/phase-gate.sh` | `_validate_inputs()` 関数を追加し全公開関数から呼ぶ |
...
**優先度**: P2-P0（IMP-checklist 形式要件 / `.kiro/specs/kairo-v1-harness/IMP-checklist.md` の `[ ] ロールバック計画` チェック解消）  
...
**優先度**: P2-P1  
**目的**: `_check_phase_specific()` の Bash `case` 文を `.kairo/config/phases.json` への `jq` 参照に移行し、フェーズ追加・変更時の変更箇所を 1 ファイルに集約する。
...
| ファイル | 操作 | 内容 |
|---------|------|------|
| `.kairo/config/phases.json` | 新規作成 | フェーズ定義 JSON |
| `.kairo/lib/phase-gate.sh` | 更新 | `_check_phase_specific()` を `jq` 参照に変更 |
...
5. 整合性確認: /kairo:coherence-scan
```

---

### T16: `req:kairo-v3-harness` ノード登録（coherence Amber 解消）

**優先度**: P2-P2  
**目的**: `specs/kairo-v1-harness/issue-struct.md` を新規作成し `coherence.id: "req:kairo-v3-harness"` を付与する。coherence-scan の dangling ref 2 件を解消し Amber 3 → 1 に削減する。
...
**影響ファイル**: `.kairo/hooks/post-tool-use.sh`, `.kairo/lib/phase-gate.sh`
...
| T05 → T04 | Actions が `.kairo/agents/*.md` でエージェントを起動する |
...
詳細手順: `commands/rescue.md`（`/kairo:rescue <issue-number>` として実行可）
...
|--------|------------|------------|-----------|
| **T01** | `docs/unified-framework-spec-v2.0.md` | `.vckd/config.yaml`, `.kairo/hooks/post-tool-use.sh`, `.claude/settings.json`, `graph/baton-log.json`, `graph/coherence.json`, `commands/install.md`, `commands/baton-status.md` | `gh label create` × 15 |
| **T02** | `commands/imp_generate.md`（既存）, `commands/implement.md`（既存）, `commands/rev.md`（既存） | 同ファイル（更新）, `.kairo/templates/imp-template.md` | なし |
| **T03** | `.vckd/config.yaml`, `graph/baton-log.json`, `graph/coherence.json` | `.kairo/lib/phase-gate.sh`, `.kairo/lib/on-label-added.sh`, `graph/baton-log.json`（実行時） | `gh issue edit`, `gh issue comment`, `gh issue view`（実行時） |
| **T04** | `commands/spec-req.md`, `commands/spec-design.md`, `commands/spec-tasks.md`, `commands/imp_generate.md`, `commands/implement.md`, `docs/unified-framework-spec-v2.0.md §3.4` | `.kairo/agents/requirements-agent.md`, `.kairo/agents/design-agent.md`, `.kairo/agents/implement-agent.md` | なし |
| **T05** | `.kairo/agents/*.md`（T04 成果物）, `.vckd/config.yaml` | `.github/workflows/vckd-pipeline.yml` | なし（ワークフロー定義のみ） |
| **T06** | `commands/test.md`, `commands/review.md`, `commands/rev.md`, `commands/drift_check.md`, `commands/sync.md`, `commands/pr.md`, `docs/unified-framework-spec-v2.0.md §3.4 #11〜21` | `.kairo/agents/test-agent.md`, `.kairo/agents/adversary-agent.md`, `.kairo/agents/ops-agent.md`, `.kairo/agents/change-agent.md` | なし |
...
| **T08** | `.kairo/lib/phase-gate.sh`（T03 成果物）, `commands/install.md`（T01 成果物） | 同ファイル（更新） | なし |
| **T09** | `.kairo/agents/*.md`（T04, T06 成果物）, `graph/baton-log.json` | `commands/rescue.md`, `.kairo/agents/*.md`（更新）, `graph/baton-log.json`（スキーマ拡張） | `gh issue edit`（rescue 実行時） |
...
|--------|-------|------|----------|-----------|
| **T01** | [#1](../../issues/1) Baton Infrastructure | P0 | ✅ 完了 | `.vckd/config.yaml`, `.kairo/hooks/post-tool-use.sh`, `graph/baton-log.json`, `graph/coherence.json` |
| **T02** | [#2](../../issues/2) CEG frontmatter 標準化 | P0 | ✅ 完了 | `.kairo/templates/imp-template.md`, `commands/imp_generate.md`（frontmatter step 追加済み） |
| **T03** | [#3](../../issues/3) Phase Gate ロジック | P0 | ✅ 完了 | `.kairo/lib/phase-gate.sh`, `.kairo/lib/on-label-added.sh` |
| **T04** | [#4](../../issues/4) Phase Agent SP（REQ/TDS/IMP） | P0 | ✅ 完了 | `.kairo/agents/requirements-agent.md`, `design-agent.md`, `implement-agent.md` |
| **T05** | [#5](../../issues/5) GitHub Actions 統合 | P1 | ✅ 完了 | `.github/workflows/vckd-pipeline.yml` |
| **T06** | [#6](../../issues/6) Phase Agent SP（TEST/OPS/CHANGE） | P1 | ✅ 完了 | `.kairo/agents/test-agent.md`, `adversary-agent.md`, `ops-agent.md`, `change-agent.md` |
...
| **T13** | [#11](https://github.com/kava2108/kairo/issues/11) `phases.json` 外部化 | P1 | 🔲 未着手 | drift D1 恒久解消 |
| **T14** | [#12](https://github.com/kava2108/kairo/issues/12) `vckd-pipeline.yml` timeout-minutes | P1 | ✅ 完了（既適用） | `run-phase-agent` に `timeout-minutes: 350` は既に設定済みと確認 |
| **T15** | [#13](https://github.com/kava2108/kairo/issues/13) 中間状態ロールバック手順 | P1 | ✅ 完了 | `commands/rescue.md` に `## 中間状態のリカバリー手順` セクションを追記 |
| **T16** | [#15](https://github.com/kava2108/kairo/issues/15) `req:` ノード登録 | P2 | ✅ 完了 | `issue-struct.md` 新規作成、coherence Amber 3→1（Green: 24→27） |
...
```bash
# 各 Agent ファイルの内容確認
/kairo:implement kairo-v3-harness T01 --issue 1   # 未確認の実装補完
/kairo:test kairo-v3-harness --vmodel all
/kairo:review kairo-v3-harness --adversary
```

---

*IMP v1.2.0 更新完了（2026-04-05）。次フェーズへ進む場合は `/kairo:test kairo-v3-harness --vmodel all` を実行してください。*
