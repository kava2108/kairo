---
kairo:
  node_id: "design:kairo-v3-harness"
...
# 技術計画書（TDS）: kairo v3.0 Harness Engineering 統合
...
graph TB
    ...
    subgraph Harness["⚙️ Harness Layer（環境設計）"]
        ...
        HK["Hook Layer<br/>.kairo/hooks/post-tool-use.sh<br/>OR .github/workflows/vckd-pipeline.yml"]
    ...
    subgraph Command["📝 Command Layer（kairo slash commands）"]
...
| **Hook Layer (A)** | `.kairo/hooks/post-tool-use.sh` | Claude Code PostToolUse フックでバトン発行 | REQ-001-AC-1 |
...
| **Agent Layer** | `.kairo/agents/*.md` | Phase Agent のシステムプロンプト | REQ-007 全 AC |
...
sequenceDiagram
    ...
    AgentLayer->>CommandLayer: /kairo:spec-req 実行
...
【kairo 責務の境界】
  ...
  IN:  Phase Agent のシステムプロンプト（.kairo/agents/*.md）
...
### 2.1 CLI API（kairo slash commands）

kairo の「API」はすべて Claude Code スラッシュコマンドとして提供される。
...
```
コマンド基本形式:
  /kairo:<command-name> [<issue-id>] [--flag <value>]
...
escalate_comment_schema:
  ...
  retry_count: integer
  last_error: string
  suggested_action: "ローカルで /kairo:rescue を実行してください"
```
...
codd:
  cli_path: string|null  # null = kairo 独自実装を使用
```
...
```
v1.0 → v2.0（kairo v3.0）へのマイグレーション:

1. .kairo/config.json が存在する場合:
   → .vckd/config.yaml に自動変換（/kairo:install --migrate）
   → 変換後も .kairo/config.json は残す（後方互換）

2. 既存 specs/<issue-id>/ のファイルへの baton frontmatter 付与:
   → /kairo:coherence-scan --backfill で既存ファイルに追記
   → 既存内容は変更しない（frontmatter を先頭に追加するのみ）

3. graph/ ディレクトリの新規作成:
   → /kairo:install --harness で自動作成
...
主フロー（AUTO_STEP=false）:
  ...
  3. RequirementsAgent が /kairo:spec-req を実行
...
主フロー:
  ...
  2. 修正コマンドを実行（例: /kairo:imp_generate --update）
  3. 成果物を修正
...
注意:
  blocked:xxx ラベルは手動で削除する（または /kairo:rescue が自動削除）
```
...
主フロー:
  1. TestAgent が /kairo:review --adversary を実行
...
出力:
  ...
  - .kairo/hooks/post-tool-use.sh（dispatch_baton 実装）
...
出力:
  ...
  - .kairo/templates/imp-template.md（frontmatter 込み）
依存: なし（T01 と並列可能）
AC: REQ-005-AC-1
...
入力:
  ...
  - §5.4 ラベル完全一覧
出力:
  - .kairo/lib/phase-gate.sh（check_phase_gate, dispatch_baton, emit_pending, emit_baton, emit_blocked）
  - .kairo/lib/on-label-added.sh（approve ラベル検知 → バトン発行）
...
出力:
  - .kairo/agents/requirements-agent.md
  - .kairo/agents/design-agent.md
  - .kairo/agents/implement-agent.md
依存: なし
AC: REQ-007-AC-1〜2
...
出力:
  - .kairo/agents/test-agent.md
  - .kairo/agents/adversary-agent.md
  - .kairo/agents/ops-agent.md
  - .kairo/agents/change-agent.md
依存: T04（設計パターンを継承）
...
出力:
  - .kairo/lib/phase-gate.sh（enabled=false 時の早期リターン追加）
...
出力:
  ...
  - .kairo/agents/ 全ファイルへのリトライロジック追記
依存: T04, T06
AC: REQ-007-AC-2
_Requirements: REQ-007_
...
| **REQ-007-AC-1** | WHEN RequirementsAgent が起動する THEN `spec-req` を実行して `requirements.md` を生成する | e2e | .kairo/agents/requirements-agent.md | §4.2（UC-001）, T04 |
| **REQ-007-AC-2** | WHEN エージェントが 3 回リトライして失敗する THEN `blocked:escalate` ラベルを付与する | integration | .kairo/agents/*.md のリトライポリシー | §4.2（UC-006）, T09 |
| **REQ-008-AC-1** | WHEN TEST→OPS Phase Gate が実行される THEN Adversarial Review が 5 次元で評価される | integration | .kairo/agents/adversary-agent.md + commands/review.md | §4.2（UC-004）, T06 |
...
*TDS 生成完了 — 本書を入力として /kairo:issue-generate kairo-v3-harness --wave P0 を実行し、P0 Issue 群を生成してください。*
