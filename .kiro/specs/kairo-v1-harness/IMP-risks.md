...
|---|-----------|--------|---------|--------|------|---------|
| R-001 | `gh` CLI 認証失敗（トークン期限切れ・権限不足）により `emit_baton` / `emit_blocked` が実行できない | H | M | **High** | exponential backoff 3 回リトライ → `blocked:escalate` 付与。`/kairo:rescue` で回復 | T03 実装者 |
