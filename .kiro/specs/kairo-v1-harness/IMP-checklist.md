...
# IMP レビューチェックリスト: kairo-v3-harness
...
- [x] **issue_id** が issue-struct.md の issue_id と一致している
  > issue-struct.md は kiro 統合のため不在（既知の例外）。全成果物で "kairo-v3-harness" に統一されていることを確認済み。
...
- [x] ワークフローの変更ファイル一覧に `.github/workflows/vckd-pipeline.yml` が含まれている
  > specs/kairo-v1-harness/implements/T05/patch-plan.md の変更対象ファイルに `.github/workflows/vckd-pipeline.yml` が含まれていることを確認。
