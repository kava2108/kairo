...
| `/kairo:design-system <product-description> [--stack <stack>] [--page <page-name>] [--persist] [--no-uupm] [--feature <feature>]` | プロダクトの自然言語説明から UI/UX デザインシステムを生成し `design-system/MASTER.md` を出力。UUPM 推論でカラー・タイポグラフィ・コンポーネントパターンを自動決定し CEG ノードとして管理 |
...
| `/tsumigi:pr` | `/kairo:pr` |
| ─ | `/kairo:design-system`（kairo 独自・UUPM 統合） |
