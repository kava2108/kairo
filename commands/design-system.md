...
description: >
  ...
  Claude ネイティブ推論で業界カテゴリ・5次元デザイン推論を実行し、
...
として永続化する。Claude ネイティブ推論で業界カテゴリ認識・5次元デザイン推論を実行し、
...
# step2: Claude ネイティブ推論（業界カテゴリ × 5次元デザイン）

> **v1.1 方針**: 外部ツール・Python スクリプトは使用しない。
> Claude 自身が業界知識に基づいてデザインシステムを推論する。
...
- **Anti-patterns**: 避けるべき UX アンチパターン（業界固有タブーを含む）
...
> Generated by kairo `/kairo:design-system` — Claude ネイティブ推論
...
<!-- 業界固有アンチパターン（金融にAIパープル不可、医療に過度なダークモード不可 等） -->
