...
- `specs/{{issue_id}}/IMP.md` の存在を確認する
  - 存在しない場合：「先に `/kairo:imp_generate {{issue_id}}` を実行してください」と言って終了する
...
## step6.5: デザインシステムコンテキスト読み込み（UI 実装時のみ）

IMP.md の変更スコープに UI コンポーネントが含まれているか判定する。
判定基準: ファイルパスに `component`, `page`, `view`, `ui`, `layout`, `style` が含まれる、または IMP.md に "UI", "画面", "コンポーネント", "デザイン" が含まれる。

**UI 実装と判定された場合**:

1. `design-system/MASTER.md` が存在するか確認する。
   - **存在する**: ファイルを Read する。MASTER.md の内容を実装の制約として適用する。
     - カラーパレット（`--color-*` トークン）
     - タイポグラフィスケール（`--text-*` トークン）
     - スペーシングシステム（`--spacing-*` トークン）
     - コンポーネントパターン
   - **存在しない**: ⚠️ 警告を表示する:
     ```
     ⚠️  design-system/MASTER.md が見つかりません。
     UI 品質を担保するには先に以下を実行することを推奨します:
       /kairo:design-system "<プロダクト説明>" --feature <feature-name>
     スキップして実装を続けますか？ (続行する場合は任意の key を入力)
     ```
     AskUserQuestion でスキップ確認を行う。スキップ時は警告ログのみ残して実装を続行する。

2. ページ固有オーバーライドを確認する（`design-system/pages/` ディレクトリを Glob）。
   タスクが特定ページに関連する場合は該当オーバーライドも Read する。
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/patch-plan-template.md`
  - `.claude/commands/kairo/templates/patch-plan-template.md`
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/impl-memo-template.md`
  - `.claude/commands/kairo/templates/impl-memo-template.md`
...
詳細確認: /kairo:drift_check {{issue_id}}
```
...
- 以下を表示する：
  ...
  次のステップ:
    テスト生成:      /kairo:test {{issue_id}} {{task_id}}
    乖離確認:        /kairo:drift_check {{issue_id}}
    全タスク確認:    /kairo:sync {{issue_id}} --report-only
  ```
...
- Bash で Issue に完了を通知する：
  ...
  **次のステップ**: `/kairo:test {{issue_id}} {{task_id}}`
  EOF
  )" 2>/dev/null
  ```
