...
- $ARGUMENTS を解析する：
  ...
  - issue_number が未指定または整数でない場合は「Issue 番号を指定してください（例: /kairo:rescue 42）」と言って終了する
- `graph/baton-log.json` が存在するか確認する
  - 存在しない場合は「VCKD Harness がセットアップされていません。`/kairo:install --harness` を実行してください」と表示して終了する
...
- 以下を表示する：
  ...
  Phase Agent の出力を確認:
    /kairo:baton-status {{issue_number}}

  ⚠️ 同じ箇所で再度 escalate になる場合は、
     システムプロンプト（.kairo/agents/*.md）の修正が必要です。
  ```
...
増加している場合は `/kairo:drift_check` で乖離の原因を特定してください。
