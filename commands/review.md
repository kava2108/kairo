...
argument-hint: "[issue-id] [--persona arch|security|qa|ui|all] [--adversary] [--pr <pr-number>]"
---
...
- persona が未設定 または "all" 以外の場合、AskUserQuestion ツールを使って質問する：
  ...
  - options:
    ...
    - label: "ui（UI/UX 品質）"
      description: "デザインシステム準拠・アクセシビリティ・UX アンチパターンを重視したレビュー"
    - label: "all（全ペルソナ）"
      description: "4つのペルソナ全てでレビューする"
...
### UI/UX レビュー（persona: ui または all）

`design-system/MASTER.md` が存在する場合はそれを Read する。存在しない場合は汎用 UI チェックリストで代替する。

**チェック項目**:

| # | カテゴリ | チェック内容 |
|---|----------|-------------|
| U01 | カーソル | 全クリック要素に `cursor: pointer` が設定されているか |
| U02 | ホバー | 全インタラクティブ要素に 150–300ms transition があるか |
| U03 | フォーカス | キーボードナビ向けの focus-visible スタイルがあるか |
| U04 | レスポンシブ | 375/768/1024/1440px の各ブレークポイントで崩れないか |
| U05 | モーション | `prefers-reduced-motion` が考慮されているか |
| U06 | コントラスト | WCAG 2.1 AA（4.5:1）を満たすカラー設計か |
| U07 | カラー | MASTER.md のカラートークンが実装に適用されているか |
| U08 | タイポグラフィ | MASTER.md のタイポグラフィスケールに準拠しているか |
| U09 | スペーシング | 4px グリッドに基づくスペーシングが使われているか |
| U10 | アンチパターン | MASTER.md の Anti-patterns セクションに記載の業界固有アンチパターンを违反していないか |

**FAIL 判定基準**: U01–U06 のいずれか未達成、または MASTER.md が存在しカラー/タイポスケール違反がある場合。
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/review-checklist-template.md`
  - `.claude/commands/kairo/templates/review-checklist-template.md`
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/risk-matrix-template.md`
  - `.claude/commands/kairo/templates/risk-matrix-template.md`
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/review-questions-template.md`
  - `.claude/commands/kairo/templates/review-questions-template.md`
...
- 以下を表示する：
  ...
  PR 作成がまだの場合:
    /kairo:pr {{issue_id}} --post-checklist

  より厳格な品質ゲートが必要な場合:
    /kairo:review {{issue_id}} --adversary
  ```
...
### 次元 6: Design Fidelity（デザイン忠実性）

**問い**: UI 実装が `design-system/MASTER.md` に定義されたデザインシステムに準拠しているか？

**前提チェック**:  
`design-system/MASTER.md` が存在しない場合、この次元は **SKIP**（自動 PASS）とする。

評価手順:
1. `design-system/MASTER.md` を Read する（UUPM 生成のデザインシステム）。
2. 実装コードから以下を Grep して MASTER.md との一致を確認する:
   - カラートークン（`var(--color-`, `#` で始まるカラーコード）
   - フォントファミリー・フォントサイズ
   - スペーシング値（`px`, `rem`, `em`）
3. MASTER.md の Pre-delivery Checklist の全項目が満たされているかを確認する。
4. 以下のいずれかに該当すれば FAIL:
   - MASTER.md に定義されていないカラーコードが実装に存在する
   - MASTER.md のタイポグラフィスケール以外のフォントサイズが使われている
   - Pre-delivery Checklist の必須項目（U01–U06）が未達成
   - MASTER.md の Anti-patterns セクションに記載された業界固有アンチパターンが実装に存在する

**判定**: PASS / FAIL / SKIP（MASTER.md なし）
FAIL の場合: MASTER.md との乖離箇所を具体的に示し、修正提案を記載する。
...
│ 6. Design Fidelity         │ PASS/FAIL/SKIP │ (要約)         │
└────────────────────────────┴────────┴─────────────────────────┘
...
**全体判定のルール**:
- 全次元が PASS または SKIP → 全体 PASS
- 1 次元でも FAIL → 全体 FAIL（例外なし）
- 次元 6 の SKIP（MASTER.md なし）は PASS として扱う

全体 FAIL の場合、FAIL 次元ごとに**自動ルーティング先**を決定する：

| FAIL 次元 | ルーティング先 | 推奨コマンド |
|---|---|---|
| Spec Fidelity | 実装の修正 | `/kairo:implement {{issue_id}} --update` |
| Edge Case Coverage | テストの追加 | `/kairo:test {{issue_id}} {{task_id}}` |
| Implementation Correctness | 実装の修正 | `/kairo:implement {{issue_id}} --update` |
| Structural Integrity | IMP の設計見直し | `/kairo:imp_generate {{issue_id}} --update` |
| Verification Readiness | テストの修正 | `/kairo:test {{issue_id}} {{task_id}}` |
| Design Fidelity | デザインシステム修正 | `/kairo:design-system "<説明>" --feature {{feature}} --persist` |
...
| 6. Design Fidelity | PASS/FAIL/SKIP | N |
...
### 次元 6: Design Fidelity — PASS/FAIL/SKIP

[具体的な指摘内容。MASTER.md なしの場合は SKIP]

## 自動ルーティング

FAIL に対して以下のアクションが必要です：

1. [アクション内容] → `/kairo:コマンド {{issue_id}}`
