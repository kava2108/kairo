---
description: 自然言語入力を kairo コマンドにルーティングします。ユーザーが何をしたいかを分析し、適切な /kairo コマンドを提案・実行します。「何をすべきか教えて」にも対応します。
...
- `.kairo/config.json` を存在する場合に Read する
...
→ 提案: `/kairo:imp_generate <issue-id>`

**IMP 更新に関するクエリ**:
- 「IMP を更新したい」「IMP を直したい」「計画が変わった」
→ 提案: `/kairo:imp_generate <issue-id> --update`
...
- 「〜を作って」「〜を実装する」
→ 提案: `/kairo:implement <issue-id> [--mode tdd|direct]`

**特定タスクの実装**:
- 「TASK-XXX を実装して」「最初のタスクから始めて」
→ 提案: `/kairo:implement <issue-id> TASK-XXX`

**乾燥確認なしの実装**:
- 「まずどんな変更か見てみたい」「ドライランで」
→ 提案: `/kairo:implement <issue-id> --dry-run`

---

**テストに関するクエリ**:
- 「テストを作って」「テストケースを洗い出して」「テスト方針を決めて」
→ 提案: `/kairo:test <issue-id>`

**テスト実行に関するクエリ**:
- 「テストを実行して」「テストして結果を確認したい」
→ 提案: `/kairo:test <issue-id> --exec`

**セキュリティテスト**:
- 「セキュリティテストを追加して」「脆弱性を確認したい」
→ 提案: `/kairo:test <issue-id> --focus security`
...
→ 提案: `/kairo:sync <issue-id> --report-only`

**自動修正も含めた同期**:
- 「整合性を直して」「sync して修正も」
→ 提案: `/kairo:sync <issue-id> --fix`
...
→ 提案: `/kairo:review <issue-id> --persona all`

**特定ペルソナのレビュー**:
- 「セキュリティ観点でレビューして」「セキュリティを確認して」
→ 提案: `/kairo:review <issue-id> --persona security`
- 「アーキテクチャ観点で」「設計をレビューして」
→ 提案: `/kairo:review <issue-id> --persona arch`
- 「テスト観点で」「品質観点で」
→ 提案: `/kairo:review <issue-id> --persona qa`
- 「UI/UX の観点で」「デザインが正しいか確認して」「UI 品質をチェックして」
→ 提案: `/kairo:review <issue-id> --persona ui`

**デザイン乖離確認**:
- 「デザインの乖離を確認したい」「UI が仕様通りか確認して」「D6 をチェックして」
→ 提案: `/kairo:review <issue-id> --adversary`（D6 Design Fidelity 含む）

**PR レビュー**:
- 「PR XXX をレビューして」「PR のチェックリストを作って」
→ 提案: `/kairo:review <issue-id> --pr <pr-number>`

---

**デザインシステムに関するクエリ**:
- 「デザインシステムを作りたい」「UI のトンマナを決めたい」「スタイルガイドを生成して」
→ 提案: `/kairo:design-system "<プロダクト説明>" --feature <feature>`
- 「カラーパレットを決めたい」「フォントを選んで」「ブランドカラーを設定して」
→ 提案: `/kairo:design-system "<プロダクト説明>"`
- 「ページごとのデザインを決めたい」「ランディングページのスタイルを設定したい」
→ 提案: `/kairo:design-system "<説明>" --page <page-name>`
- 「UIを実装したい」「コンポーネントを作りたい」（デザインシステムがある場合）
→ 提案: `/kairo:implement <issue-id>`（MASTER.md を自動参照）

---

**インストール・セットアップに関するクエリ**:
- 「kairo を使い始めたい」「セットアップして」「初期化して」
→ 提案: `/kairo:install`
...
- 現在の進捗を表示する：
  ...
  次の推奨コマンド:
    /kairo:implement {{issue_id}} TASK-0002
  ```
