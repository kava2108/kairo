...
- $ARGUMENTS がない場合は「引数に issue-id を指定してください（例: /kairo:issue_init 001-feature-name）」と言って終了する
- $ARGUMENTS を解析する：
...
- 以下のファイルを存在する場合に読み込む：
  - `docs/kairo-context.md`（プロジェクト技術スタック概要）
  - `.kairo/config.json`（kairo 設定）
  - `KAIRO.md`（プロジェクト固有ワークフロー）
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/issue-struct-template.md`
  - `.claude/commands/kairo/templates/issue-struct-template.md`
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/tasks-template.md`
  - `.claude/commands/kairo/templates/tasks-template.md`
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/note-template.md`
  - `.claude/commands/kairo/templates/note-template.md`
...
- Bash で以下を実行する：
  ...
  **次のステップ**: `/kairo:imp_generate {{issue_id}}`
  EOF
  )" 2>/dev/null
  ```
