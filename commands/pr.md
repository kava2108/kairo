...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/pr-body-template.md`
  - `.claude/commands/kairo/templates/pr-body-template.md`
- テンプレートの以下の変数を置換する：
...
- `specs/{{issue_id}}/review-checklist.md` が存在するか確認する
  - 存在しない場合：「レビューチェックリストが未生成です。`/kairo:review {{issue_id}}` を先に実行することを推奨します」と案内してスキップする
...
- 以下を表示する：
  ...
  次のステップ:
    レビュー資料の生成（未実施の場合）:
      /kairo:review {{issue_id}} --pr {{pr_number}}
    マージ前の最終確認:
      /kairo:drift_check {{issue_id}}
      /kairo:sync {{issue_id}} --report-only
  ```
