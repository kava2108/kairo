...
argument-hint: "[project-name] [--lang ja|en] [--speckit] [--harness] [--skip-uupm]"
---
...
## step9: UUPM（ui-ux-pro-max-skill）セットアップ

`--skip-uupm` フラグが指定されている場合はこのステップをスキップする。
...
- Python 3.8 以上が利用可能な場合: 次のステップへ進む。
- 利用不可の場合: 以下を表示して 9.3 へスキップ:
  ```
  ⚠️  Python 3.x がいません。UUPMの自動セットアップをスキップします。
      後と1. Python 3 をインストールしてから /kairo:install --skip-harness を再実行してくダさい。
  ```

**9.2 UUPM スキルの配置**:

- Bash で UUPM リポジトリを `.claude/skills/ui-ux-pro-max/` にクローンする:
...
if [ ! -d ".claude/skills/ui-ux-pro-max" ]; then
  ...
  echo "UUPM_ALREADY_EXISTS"
fi
```

- クローン後、`.claude/skills/ui-ux-pro-max/.git` ディレクトリを削除する（サブモジュールにしない）:

```bash
rm -rf .claude/skills/ui-ux-pro-max/.git
```

**9.3 UUPM 依存ライブラリの確認**:

```bash
python3 -c "import json, os; print('OK')" 2>/dev/null || echo "DEPS_MISSING"
```

- `DEPS_MISSING` の場合: 以下を表示する:
  ```
  ⚠️  UUPM に必要な Python 標準ライブラリを確認できませんでした。
      /kairo:design-system 実行時に再度確認します。
  ```

**9.4 `.gitignore` への追加**:

`.gitignore` に以下が含まれていない場合は追記する:

```
# UUPM Skill (プロジェクトが管理する場合は削除)
.claude/skills/ui-ux-pro-max/
```
...
（UUPM セットアップ時は以下も追加）
  .claude/skills/ui-ux-pro-max/    ← UUPM 推論エンジン
