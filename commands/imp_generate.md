...
- $ARGUMENTS がない場合は「引数に issue-id を指定してください（例: /kairo:imp_generate 001-feature-name）」と言って終了する
- $ARGUMENTS を解析する：
...
- `specs/{{issue_id}}/issue-struct.md` の存在を確認する
  - 存在しない場合：「先に `/kairo:issue_init {{issue_id}}` を実行してください」と言って終了する
...
`.kairo/templates/IMP-template.md` が存在する場合はそれをベースにする。
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/IMP-checklist-template.md`
  - `.claude/commands/kairo/templates/IMP-checklist-template.md`
...
- テンプレートを Read する（以下の順で探索し、最初に見つかったものを使用する）：
  - `~/.claude/commands/kairo/templates/IMP-risks-template.md`
  - `.claude/commands/kairo/templates/IMP-risks-template.md`
...
- 以下を表示する：
  ...
  次のステップ:
    実装を開始する:  /kairo:implement {{issue_id}}
    乾燥確認する:    /kairo:drift_check {{issue_id}}
